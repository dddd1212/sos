...
#define NUM_OF_PAGES(bytes) ((bytes + PAGES_SIZE-1)/PAGES_SIZE)
...
struct STAGE0BootModules {
    ...
    Eff * file_data; // data of the eff file.
...
void _start(void * my_address) {
    ...
    //char * boot_txt_ptr = boot_txt_data;
    char * boot_txt_end;
    char * line;
    char boot_txt_file_name[] = {'b','o','o','t','.','t','x','t','\x00'};
    unsigned int boot_txt_file_size;
    
    int alloc_pages = NUM_OF_PAGES(sizeof(KernelGlobalData)) + // the kgd will be the first in the memory
    ...
    while (boot_txt_data[i] != '\x00') {
        if (line >= MAX_BOOT_MODULES) {
            STAGE0_suicde(0x3000);
        }
        boot_modules[line]->filename = &boot_txt_data[i]; // File
        line++;
    ...
    kgd = (KernelGlobalData *) STAGE0_virtual_commit(alloc_pages);
    ret = STAGE0_virtual_pages_alloc(kgd, alloc_pages, PAGE_ACCES_RW);
    if (kgd == NULL || ret == NULL) {
        STAGE0_suicde(0x5000);
    }
    STAGE0_memset(kgd, 0, sizeof(KernelGlobalData) + sizeof(ModulesList));
    ...
    for (line = 0 ; line < num_of_lines ; line++) {
        ...
        // points to the data:
        boot_modules[line]->file_data = (Eff*)modules_addr;
        
        // move to the next address
        modules_addr += PAGE_SIZE * boot_modules[line]->file_pages;
    }
    ...
    STAGE0_suicde(0xffffffff);    
}

void * STAGE0_find_symbol(KernelGlobalData * kgd, char * symbol_name) {
...
void * STAGE0_virtual_commit(uint32 numOfPages) {
    return 0;
}

// allocate commited pages.
int STAGE0_virtual_pages_alloc(void* requestedAddr, uint32 numOfPages, PAGE_ACCESS access) {
    return 0;
}
